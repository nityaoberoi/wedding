...
i.error(m)}}})}return this}function a(i,k){this.each(function(){for(var l=f(this),m,q,r=k.indexOf("<img src=");r!=-1;){r+=10;q=k.indexOf('"',r);m=i.skinDir+"/"+k.substring(r,q);k=k.substr(0,r)+m+k.substr(q);r=k.indexOf("<img src=",r)}m=window.location.host.split(":")[0];m=m.split(".").slice(-2).join(".");if(m=="tn3gallery.com"){l.append(k);l.data("tn3").init(l,i.fullOnly)}})}function b(i){var k=[],l=i.children(".tn3.album"),m,q;if(l.length>0)l.each(function(r){m=f(this);k[r]={title:m.find(":header").html()};
f.extend(k[r],c(m));if(q=d(m)){k[r].imgs=q;if(!k[r].thumb)k[r].thumb=k[r].imgs[0].thumb}});else if(q=d(i))k[0]={imgs:q};return k}function d(i){var k=[],l,m,q;l=i.find("li");if(l.length>0)l.each(function(r){m=f(this);q=m.find(":header");k[r]={title:q.html(),img:m.find("a").attr("href"),thumb:m.find("a img").attr("src")};if(!k[r].img)k[r].img=m.find("img").attr("src");f.extend(k[r],c(m))});else{l=i.find("img");l.each(function(r){m=f(this);$at=m.parent("a");k[r]=$at.length==0?{title:m.attr("title"),
img:m.attr("src")}:{title:m.attr("title"),img:$at.attr("href"),thumb:m.attr("src")}})}if(k.length==0)return null;return k}function c(i){var k={};i=i.children(".tn3");var l;f.each(i,function(){l=f(this);k[l.attr("class").substr(4)]=l.html()});return k}function g(i){f('a[href^="#tn3-'+i+'"]').click(function(k){var l=j[i];k=f(k.currentTarget).attr("href");k=k.substr(k.indexOf("-",5)+1);k=k.split("-");switch(k[0]){case "next":l.cAlbum!=null&&l.show("next",k[1]=="fs");break;case "prev":l.cAlbum!=null&&
l.show("prev",k[1]=="fs");break;default:l.cAlbum!=parseInt(k[0])?l.showAlbum(parseInt(k[0]),parseInt(k[1]),k[2]=="fs"):l.show(parseInt(k[1]),k[2]=="fs")}})}function h(){if(j.length==0){var i=f(".tn3gallery");i.length>0&&i.tn3({})}}if(/1\.(0|1|2|3|4)\.(0|1)/.test(f.fn.jquery)||/^1.1/.test(f.fn.jquery)||/^1.2/.test(f.fn.jquery)||/^1.3/.test(f.fn.jquery))alert("tn3gallery requires jQuery v1.4.2 or later!  You are using v"+f.fn.jquery);else{var p={},j=[];f.fn.tn3=function(i){i=f.extend(true,{},f.fn.tn3.config,
i);if(i.skin!=null)if(typeof i.skin=="object"){i.skinDir+="/"+i.skin[0];if(i.cssID==null)i.cssID=i.skin[0];i.skin=i.skin[1]}else i.skinDir+="/"+i.skin;else{i.skin="tn3";i.skinDir+="/tn3";var k=true}if(i.cssID==null)i.cssID=i.skin==null?"tn3":i.skin;this.each(function(){var l=f(this);i.fullOnly?l.hide():l.css("visibility","hidden");var m=i.data?i.data:b(l);m=j.push(new f.fn.tn3.Gallery(m,i))-1;l.data("tn3",j[m]);for(var q=0;q<f.fn.tn3.plugins.length;q++)f.fn.tn3.plugins[q].init(l,i);l.empty();g(m)});
k?a.call(this,i,i.skinDefault):e.call(this,i);return this};f.fn.tn3.plugins=[];f.fn.tn3.plugIn=function(i,k){f.fn.tn3.plugins.push({id:i,init:k})};f.fn.tn3.version="1.1.0.34";f.fn.tn3.config={data:null,skin:null,skinDir:"skins",skinDefault:'<div class="tn3-gallery"><div class="tn3-image"><div class="tn3-text-bg"><div class="tn3-image-title"></div><div class="tn3-image-description"></div></div><div class="tn3-next tn3_v tn3_o"></div><div class="tn3-prev tn3_v tn3_o"></div><div class="tn3-preloader tn3_h tn3_v"><img src="preload.gif"/></div><div class="tn3-timer"></div></div><div class="tn3-controls-bg tn3_rh"><div class="tn3-sep1"></div><div class="tn3-sep2"></div><div class="tn3-sep3"></div></div><div class="tn3-thumbs"></div><div class="tn3-fullscreen"></div><div class="tn3-show-albums"></div><div class="tn3-next-page"></div><div class="tn3-prev-page"></div><div class="tn3-play"></div><div class="tn3-count"></div><div class="tn3-albums"><div class="tn3-inalbums"><div class="tn3-album"></div></div><div class="tn3-albums-next"></div><div class="tn3-albums-prev"></div><div class="tn3-albums-close"></div></div></div>',
...
this,this.onLoadThumb,[e,this.thumbCount]));!this.config.seqLoad&&this.thumbCount<this.data.length-1&&this.loadNextThumb();return}else this.config.mode="bullets"}this.config.mode=="numbers"&&e.text(this.thumbCount+1);this.onLoadThumb(null,e,this.thumbCount)},onLoadThumb:function(e,a,b,d){this.lis[b]={li:a};a.addClass(this.config.cssID+"-thumb");a.css("float",this.isVertical?"none":"left");if(e){var c=this.lis[b].thumb=a.append(e).find(":last");this.lis[b].pos=a.position()[this.edge]}this.config.useTitle&&
a.attr("title",this.data[b].title);if(this.config.mode=="thumbs"){this.lis[b].shade=a.prepend("<div/>").find(":first");this.lis[b].shade.css("background-color",this.config.shaderColor).css("width",c.width()).css("height",c.height()).css("position","absolute")}this.initThumb(b);a.css("opacity",0);a.animate({opacity:1},1E3);this.listSize+=a[this.outerSize](true);if(!this.initialized){this.initialized=true;this.initMouse(true)}d&&this.trigger("error",{description:d,n:b});this.trigger("thumbLoad",{n:b});
...
a.position()[this.edge],d=a[this.outerSize]()/2;if(b+d>this.containerSize||b+d<0){a=10-a.position()[this.edge]+this.containerSize/2-d;a=Math.min(0,a);a=Math.max(a,-this.listSize+this.containerSize);b={};b[this.edge]=a;e?this.$ul.css(b):this.$ul.animate(b,200)}}},thumbsLoaded:function(){this.$c.css("cursor","auto");this.$ul.css("width",this.listSize+"px");this.centerList();this.trigger("load")},centerList:function(e){if(this.listSize<this.containerSize){var a={};a[this.edge]=this.config.align?this.config.align==
...
a;j<d;j++){var i=this.data[j];if(!i.title)i.title="Unknown";c=h[j-a];g='<div class="'+this.cssID+'-album-image"></div>';g+='<div class="'+this.cssID+'-album-title">'+i.title+"</div>";if(i.description)g+='<div class="'+this.cssID+'-album-description">'+i.description+"</div>";c.html(g);new f.fn.tn3.ImageLoader(i.thumb,this,this.onThumbLoad,[c.find("."+this.cssID+"-album-image")]);c.data("tn3",j);this.trigger("binit",{$a:c});c.click(function(){var l=f(this).data("tn3");if(l==p.selected){p.$c.hide();
p.trigger("close")}else p.trigger("click",{n:l})}).hover(function(){f(this).addClass(p.cssID+"-album-over")},function(){f(this).removeClass(p.cssID+"-album-over")});j==this.selected&&c.addClass(this.cssID+"-album-selected")}c=j%h.length;if(c!=0)for(g=h.length-1;g>=c;g--)h.splice(g,1);this.n=e;if(b)f.each(h,function(l){h[l].show()});else{var k=800/h.length;this.$c.slideDown(e==0?300:0,function(){f.each(h,function(l){h[l].delay(l*k).fadeIn(150)})})}this.ctrl_prev&&this.enablePageControl("prev",a>0);
